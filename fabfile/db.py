...
def build():
    ...
    do('mkdir -p migrations/versions')

    do('rm -f migrations/versions/*.pyc')

    # Run migrations
    do('%s/bin/alembic -c migrations/alembic.ini upgrade head' % venv_path)
...
def generate(message=None):
    ...
    if message:
        args = '-m "%s"' % message
    do('%s/bin/alembic -c migrations/alembic.ini revision --autogenerate %s' % (venv_path, args))
